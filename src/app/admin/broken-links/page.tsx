...
export default function BrokenLinksPage() {
    ...
    const siteUrl = 'https://cachdautu.com';
    ...
    const extractLinks = useCallback(async () => {
        ...
        posts.forEach((post: any) => {
            ...
            extracted.forEach(link => {
                ...
                if (!isExternal) {
                    const targetSlug = link.url.replace(/^\/bai-viet\//, '').replace(/\/$/, '');
    ...
    const checkLinks = async () => {
        ...
        for (let i = 0; i < uniqueUrls.length; i++) {
            ...
            try {
                ...
                if (data.ok) {
                    statusMap.set(url, { status: data.redirected ? 'redirect' : 'ok', code: data.status });
                } else {
        ...
        setLinks(prev => prev.map(link => {
            if (link.type === 'outbound') {
                ...
                if (result) return { ...link, status: result.status, statusCode: result.code };
            }
            return link;
        }));
        setScanning(false);
                        ...
                        {filtered.slice(0, 100).map((link, idx) => (
                            <div key={idx} className="flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50/50 transition-colors">
                                ...
                                <div className="flex items-center gap-1.5 shrink-0">
                                    {link.isExternal ? (
                                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 border border-purple-200 font-medium">External</span>
                                    ) : (
                                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-sky-50 text-sky-600 border border-sky-200 font-medium">Internal</span>
                                    )}
                                    {link.isNoFollow && (
                                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 border border-gray-200 font-medium">nofollow</span>
                                    )}
                                </div>
                                <div className="flex items-center gap-1.5 shrink-0">
