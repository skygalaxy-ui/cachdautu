// scripts/fix-all-images-auth.mjs
// Sửa TẤT CẢ ảnh đại diện - có xác thực admin để truy cập toàn bộ bài viết
...
function detectTopic(title, categorySlug) {
    ...
    const rules = [
        ...
        { keywords: ['thói quen', 'quản lý tài chính', 'chi tiêu', 'ngân sách', 'nợ', 'quy tắc'], topic: 'personal-finance' },
...
async function main() {
    ...
    // Get ALL posts (with auth session now)
    ...
    needsFix.forEach((p, i) => console.log(`  ${i + 1}. [${p.reason}] ${p.title.substring(0, 60)}`));
    console.log('');
