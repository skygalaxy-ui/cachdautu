...
const verifyUrl = async (url) => {
    ...
    if (error) {
        return { url, slug, status: 'NOT_FOUND_IN_DB', error: error.message };
    }
...
fs.writeFileSync('f:/antigravity-projects/cachdautu/scripts/url-audit-results.json', JSON.stringify(results, null, 2));
console.log("Done. Results saved to url-audit-results.json");
