...
async function run() {
  ...
  const { error: err1 } = await supabase.from("posts").insert({
    ...
    content: art1,
    featured_image: "https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?q=80&w=2670&auto=format&fit=crop",
  ...
  const { error: err2 } = await supabase.from("posts").insert({
    ...
    content: art2,
    featured_image: "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?q=80&w=2670&auto=format&fit=crop",
    category_id: chungKhoanId,
    is_published: true,
    scheduled_at: new Date().toISOString()
  });
