// scripts/fix-all-images.mjs
// Sửa TẤT CẢ ảnh đại diện bị lỗi hoặc thiếu - cho toàn bộ bài viết
...
// Ảnh Unsplash ĐÃ XÁC MINH - đa dạng theo chủ đề
const VERIFIED_IMAGES = {
    ...
    'investment': [
        ...
        'photo-1565373677928-90e963765bdc',
...
function detectTopic(title, categorySlug) {
    ...
    if (categorySlug) {
        const catMap = {
            ...
            'trai-phieu': 'bonds',
            'khoi-nghiep': 'business',
            'quy-dau-tu': 'investment',
            'vang': 'gold',
            'forex': 'forex',
            'dau-tu-thay-the': 'investment',
            'kien-thuc-dau-tu': 'education',
            'kinh-nghiem': 'education',
        };
        if (catMap[categorySlug]) return catMap[categorySlug];
    }

    // Fallback: detect by title keywords
    const rules = [
        { keywords: ['etf', 'vn30', 'vnfin', 'quỹ chỉ số', 'quỹ mở'], topic: 'stock-market' },
        ...
        { keywords: ['cổ phiếu', 'chứng khoán', 'vn-index', 'mở tài khoản', 'hose', 'hnx', 'phân tích kỹ thuật', 'pe ', 'eps'], topic: 'stock-market' },
        ...
        { keywords: ['thói quen tài chính', 'quản lý tài chính', 'thu nhập', 'chi tiêu', 'ngân sách', 'nợ', 'quy tắc'], topic: 'personal-finance' },
        { keywords: ['ai ', 'công nghệ', 'fintech', 'robo'], topic: 'technology' },
        { keywords: ['warren buffett', 'peter lynch', 'kinh nghiệm', 'bài học', 'nguyên tắc', 'tâm lý', 'sai lầm'], topic: 'education' },
        { keywords: ['vàng', 'gold', 'kim loại quý'], topic: 'gold' },
        { keywords: ['forex', 'ngoại hối', 'tỷ giá'], topic: 'forex' },
        { keywords: ['đầu tư', 'rủi ro', 'danh mục', 'phân bổ', 'lợi nhuận', 'chiến lược', 'so sánh'], topic: 'investment' },
    ];
...
async function main() {
    console.log('🔍 Quét TẤT CẢ bài viết...\n');
    ...
    needsFix.forEach((p, i) => console.log(`  ${i + 1}. [${p.reason}] ${p.title}`));
    console.log('');
    ...
    for (const post of needsFix) {
        try {
            ...
            const sourceUrl = buildUnsplashUrl(photoId);
            console.log(`⬇️  [${topic}] ${post.title.substring(0, 50)}...`);
    ...
    console.log(`📊 Tổng: ${okCount + success}/${posts.length} bài có ảnh`);
    console.log('═══════════════════════════════════════');
}
