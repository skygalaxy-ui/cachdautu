...
async function check() {
    ...
    slugs.forEach((slug, index) => {
        ...
        if (match) {
            process.stdout.write(`RESULT[${index}]: ${slug} | FOUND | Pub: ${match.is_published} | Date: ${match.scheduled_at}\n`);
        } else {
            process.stdout.write(`RESULT[${index}]: ${slug} | MISSING\n`);
        }
    });
    process.stdout.write("--- END CHECK ---\n");
}
